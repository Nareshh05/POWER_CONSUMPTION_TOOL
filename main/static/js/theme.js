...
document.addEventListener("DOMContentLoaded", function () {
   ...
   if (sidebar) {
      ...
      if (links.length > 0) {
         links.forEach(function (link) {
            link.addEventListener("click", function (event) {
               if (this.getAttribute("href").charAt(0) === "#") {
                  ...
                  if (targetSection && simpleBar && simpleBar.getScrollElement()) {
         ...
         // Safely scroll to the last link (only if valid)
         if (simpleBar && simpleBar.getScrollElement()) {
            // Optional: Scroll to the last link or a specific position
            // var lastLink = links[links.length - 1];
            // simpleBar.getScrollElement().scrollTop = lastLink.offsetTop - sidebar.offsetTop;
         }

      } else {
         console.warn("No links found in the navbar.");
      }
   }
});
